...
% Retrieve intersections between the initial moon & the rest of moons
LEGS_tempinter = LEGS_inter(LEGS_inter(:,1) == depNode(1) & abs(LEGS_inter(:,3) - depNode(3))<1e-7, :);

% Find the difference in pump angles between departure node & intersection options
diffalpa       = abs(LEGS_tempinter(:,2) - alpha);
diffalpadmax   = - diffalpa + deltaMax;    % Include the bending achievable with the flyby
indxs          = find(diffalpadmax >= 0 | (diffalpadmax < 0 & (abs(diffalpadmax)) <= toldegreesI)); % --> to save!

% Retrieve the feasible intersections
if ~isempty(indxs)
...
% Compute estimated TOF for the intersections
for i = 1:size(LEGSnext_2,1)
    
    Tour = LEGSnext_2(i,:);
    idx_1 = [1:12:size(Tour, 2)];
    idx_2 = [2:12:size(Tour, 2)];
    All_Moon_IDS = Tour(:,idx_1);
    Transf_Type = Tour(:,idx_2);

    % Retrieve the orbit parameters of the intersection wrt one of the moons
    v_inf = Tour(:,end-4);
    v_sc = (v_inf^2) + 1;

    if Transf_Type(end-1) == 88 && All_Moon_IDS(end-1)>All_Moon_IDS(end)
        A = 1;
    elseif Transf_Type(end-1) == 88 && All_Moon_IDS(end-1)<All_Moon_IDS(end)
        B = 1;
    elseif Transf_Type(end-1) == 11 && All_Moon_IDS(end-1)<All_Moon_IDS(end)
        C = 1;
    elseif Transf_Type(end-1) == 11 && All_Moon_IDS(end-1)>All_Moon_IDS(end)
        D = 2;
    end
