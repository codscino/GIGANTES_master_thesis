...
%         LEGSnext = apply_MODP_tissV2(LEGSnext, INPUT); % --> MODP
%         LEGSnext = apply_MOBS_tiss(LEGSnext, INPUT); % --> MOBS
        disp(['The number of Moon tours in the Buffer is:' num2str(size(LEGSnext,1)) ]) ;
        % LEGSnext = apply_MOBS_tissV2(LEGSnext, INPUT); % --> MOBS
    end
...
%     [LEGSnext, output(indl).LEGS] = checkArrived(LEGSnext, INPUT);
     ...
     [LEGSnext] = check_Tour(LEGSnext, INPUT);
