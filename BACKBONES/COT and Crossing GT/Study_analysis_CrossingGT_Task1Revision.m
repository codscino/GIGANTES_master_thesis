...
nodeout = [vinfin, alfaou, kin-Delta_Crank_max];
% nodeout = [vinfin, alfaou, 0];
% Compute flyby parameters
[Flyby1] = Flyby_BuildUp(nodein, nodeout, pars);
%--------------------------------------------------------------------------
% PLOT GROUDNTRACK FIRST ENCOUNTER %
colors = cool(length(Flyby1));
...
for i = 1:size(Flyby1, 2)
    Plot_Flyby_GT(Flyby1(i), colors(i,:));
end
%--------------------------------------------------------------------------
%% PART TWO - Manouvre at oposite node 
KEPDeparture=Flyby1.OE_Out;
