...
clear all; close all; clc; warning off; format short g;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Definition of Starting Node before Partial COT and Crossing Ground-tracks

vinfin  = pars.INPUTS.V_inf;
alfain  = Reso_Transfers(1, 4);
kin     = pars.INPUTS.crank;

node0  = [vinfin, alfain, kin]; %[km/s, rad, rad]
...
[vvinfin, rr_in, vv_in] = vinfAlphaCrank_to_VinfCART(vinfin, alfain, kin, epoch0, pars.INPUTS.idMoon, pars.INPUTS.idCentral);
OE_0_EnceladusNode  = car2kep([rr_in, vv_in], pars.Planet.mu);

% Computing Vacant Node Conditions - Point of Manoeuvre application
OE_0_VacantNode  = OE_0_EnceladusNode;
OE_0_VacantNode(6)=OE_0_VacantNode(6)+pi;

tof2get2VacantNode = kepEq_t(OE_0_VacantNode(6), OE_0_VacantNode(1), OE_0_VacantNode(2), pars.Planet.mu, OE_0_EnceladusNode(6), 0);

SV_atVacantNode_orbit0 = kep2car(OE_0_VacantNode,pars.Planet.mu);

% computing artifitial cranking manoeuvre
v_car = SV_atVacantNode_orbit0(4:6);
v_rth = car2rth(SV_atVacantNode_orbit0(4:6),SV_atVacantNode_orbit0);

v_norm=norm(v_car);

% the logic of the artifitial cranking manoeuvre is to increase the out of
% plane component by a given amount, while keeping the same semimajor axis.
% For this, here it is chosen to reduce the radial component of the
% velocity by an amount such the velocity magnitude is the same.

DV_total=0.085; % Fixed value of manoeuvre
DV_r=-DV_total^2/2/v_rth(1);
DV_n=sqrt(DV_total^2-DV_r^2);

v_rth_new=[v_rth(1)+DV_r, v_rth(2), DV_n];

v_car_new = rth2car(v_rth_new,SV_atVacantNode_orbit0);


r1=SV_atVacantNode_orbit0(1:3);
r2=rr_in;
r1guess=v_car_new(1:3);
% Remaining Time of Flight calculation

TOF=Reso_Transfers(1, 6)-tof2get2VacantNode;

[v_car_new_corrected,v_car_new_atEnceladus]=LambertArc_IntraNode_Transfer(r1, r2, r1guess', TOF, pars.Planet.mu);

Revised_DV_manoeuvre=norm(v_car_new_corrected-v_car);


[rr_Enceladus, vv_Enceladus] = approxEphem_CC(pars.INPUTS.idMoon, epoch0, pars.INPUTS.idCentral);

vinfCAR_new_corrected=v_car_new_atEnceladus-vv_Enceladus;

[NODEIN] = car2vinfAlphaCrank(vinfCAR_new_corrected, [rr_Enceladus vv_Enceladus]);

%% INITIAL Cranking Sequence

nodein  = NODEIN; %[km/s, rad, rad]
n_crankFlybys_inSequence=5;
for indk = 1:n_crankFlybys_inSequence
    ...
    Delta_Crank_max = acos((cos(delta_max) - cos(alfain)*cos(alfain))/(sin(alfain)*sin(alfain)));
    nodeout = nodein;
    nodeout(3) = nodein(3)+Delta_Crank_max;
    % Compute flyby parameters
    [Flyby] = Flyby_BuildUp(nodein, nodeout, pars);

    % Store the results
    PCOT_crankUP(indk)  = Flyby;
    nodein  = nodeout;
end
%% Linking Cranking Up Sequence with cranking down Sequence

KEPDeparture=PCOT_crankUP(end).OE_Out; % exit orbit
KEPDeparture(6)=KEPDeparture(6)+pi;


SV_atOpositeNode = kep2car(KEPDeparture,pars.Planet.mu);
v_rth = car2rth(SV_atOpositeNode(4:6),SV_atOpositeNode);

% dvz=2*SV_atOpositeNode(6)
%
% SV_atOpositeNode(6)=SV_atOpositeNode(6)-dvz
%
% KEPInclinationChanged=KEPDeparture
% KEPInclinationChanged(3)=deg2rad(2)
%
% SV_1 = kep2car(KEPDeparture,pars.Planet.mu);
% SV_2 = kep2car(KEPInclinationChanged,pars.Planet.mu);

% one can see that the radial component is still around 600 m/s, so the
% manoeuvre to switch the crank by 180 degrees would cost on the order of
% 1.2 km/s. However, to come back to the original node would cost the same
% than the initial manouvre, followed by a pseudo-resonance and then same
% maoeuvre to reach opposite cranking.
...
for indk = 1:n_crankFlybys_inSequence
    ...
    Delta_Crank_max = acos((cos(delta_max) - cos(alfain)*cos(alfain))/(sin(alfain)*sin(alfain)));
    nodeout = nodein;
    nodeout(3) = nodein(3)-Delta_Crank_max; % Attention sign, this is crank down
    % Compute flyby parameters
    [Flyby] = Flyby_BuildUp(nodein, nodeout, pars);

    % Store the results
    PCOT_crankDOWN(indk)  = Flyby;
    nodein  = nodeout;
end
...
colors = cool(length(PCOT_crankUP));
colors2 = autumn(length(PCOT_crankDOWN));
fig1 = figure( 'Color', [1 1 1] );
hold on;
plotTextureLatLong(pars.INPUTS.idMoon , pars.INPUTS.idCentral , 1);
plotSquares(pars, 1);  
axis normal;
for i = 1:size(PCOT_crankUP, 2)
    Plot_Flyby_GT(PCOT_crankUP(i), colors(i,:));
end
for i = 1:size(PCOT_crankDOWN, 2)
    Plot_Flyby_GT(PCOT_crankDOWN(i), colors2(i,:));
end
