function [Output] = Flyby_BuildUp_claudio(nodein, nodeout, pars)
...
[vvinfin, rr_in, vv_in, ~] = vinfAlphaCrank_to_VinfCARTClaudio(vinfin, alphain, kin, epoch0, pars);

% Converting Outgoing Node into Planeto-Centric Cartesian state vector in TCN frame
[vvinfou, rr_ou, vv_ou, vvga]  = vinfAlphaCrank_to_VinfCARTClaudio(vinfou, alphaou, kou, epoch0, pars);

% Compute the flyby ground-track parameters
[states, tt4states, positions, ~, lats, longs, ~, ~, rp_lat, rp_long] = groundTrackFlyByMoonClaudio(vvinfin, vvinfou, pars.delta_max, rr_ou, vvga, pars.Moon.mu, pars);
...
Output.altitudes = positions'-pars.Moon.EquRad;
