function [ra_linked_conics, ra_kepler, ra_nbody, f_in] = calculateApoapsisComparison2(pars, spiceParam, t0_mjd, perturbingBodyNaifIDs,  propDirection, useJ2)
...
% Pre-allocate state history matrix for efficiency
num_steps = length(time_vector);
kep_state = zeros(num_steps, 6);

% Initial state vectors must be columns for FGKepler_dt2
r0_col = r0_sc'; 
v0_col = v0_sc';

% Loop through the time vector and propagate to each point
for i = 1:num_steps
    dt = time_vector(i);
    [rf, vf] = FGKepler_dt2(r0_col, v0_col, dt, mu_central_body);
    
    % Store the resulting state vector (transpose back to a row)
    kep_state(i, :) = [rf', vf'];
end

% Calculate the apoapsis from the propagated trajectory history
