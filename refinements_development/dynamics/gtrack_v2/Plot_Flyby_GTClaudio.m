function [] = Plot_Flyby_GTClaudio(Flyby_Data, color, lineStyle, width)
...
%% INPUTS %%
%
% - Flyby_Data: Structure with 16 fields containing all the information and
%               parameters of the flyby linking the Vinf, pump and crank
%               defined in the fields "nodein" and "nodeout".
%
% - color: Double of size 1 x 3 defining the color of the ground-track to
%          be plotted.
%
% - lineStyle: (Optional) Character vector specifying the line style.
%              Examples: '-' (solid), '--' (dashed), ':' (dotted),
%              '-.' (dash-dot). Default is '-'.


% Set default line style if not provided
if nargin < 3
    lineStyle = '-'; % Default to a solid line
end

% Set default line width if not provided
if nargin < 4
    width = 2; % Default to a solid line
end
...
if ~isempty(stop_index) && ~isequal(stop_index, 1)
    ...
    plot(rad2deg(Long_1), rad2deg(Lat_1), lineStyle, 'Color', color, 'LineWidth', width);
    plot(rad2deg(Long_2), rad2deg(Lat_2), lineStyle, 'Color', color, 'LineWidth', width);
    ...
    plot(rad2deg(Long_2(end)), rad2deg(Lat_2(end)), 'o', 'MarkerSize', 104, 'MarkerEdgeColor', 'black', 'MarkerFaceColor', [0.9290 0.6940 0.1250], 'MarkerSize', 5 ); % Exit point of the flyby hyperbola
...
%     plot(rp_long, rp_lat, 'o', 'MarkerEdgeColor', 'black', 'MarkerFaceColor', 'red', 'MarkerSize', 5, 'DisplayName', 'Flyby Periapsis Point');
    ...
    plot(rad2deg(longs), rad2deg(lats), lineStyle, 'Color', color, 'LineWidth', width);
