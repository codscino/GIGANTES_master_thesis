...
% =========================================================================
% This section uses the MATLAB Genetic Algorithm (ga) to find an optimal
% initial node ('nodein_nb') that meets the specified condition.
% =========================================================================
...
% --- 1. Define Target and Initial State ---
target_sma = nodeout_LC(1);
initial_sma_nb = nodeout_nb(1);
...
% --- 2. Define Bounds for Optimization Variables (nodein_nb) ---
% Create a reasonable search space around the original nodein_nb
% [a, e, i, Om, om, th]
lb = nodein_nb;
ub = nodein_nb;

% Set bounds: +/-5% for SMA and eccentricity, +/-3 degrees for angles
lb(1) = nodein_nb(1) * 0.95; % Semi-major axis
ub(1) = nodein_nb(1) * 1.05;
lb(2) = max(0, nodein_nb(2) * 0.95); % Eccentricity
ub(2) = nodein_nb(2) * 1.05;
lb(3) = nodein_nb(3) - deg2rad(3); % Inclination
ub(3) = nodein_nb(3) + deg2rad(3);
lb(4) = nodein_nb(4) - deg2rad(3); % RAAN
ub(4) = nodein_nb(4) + deg2rad(3);
lb(5) = nodein_nb(5) - deg2rad(3); % Arg of Periapsis
ub(5) = nodein_nb(5) + deg2rad(3);
lb(6) = nodein_nb(6) - deg2rad(3); % True Anomaly
ub(6) = nodein_nb(6) + deg2rad(3);
...
% Pass the original state and dV limit for the constraint check
ga_params.original_state_cart = full_state_out_nb(1,:);
...
% Total duration of the flyby propagation.
ga_params.propagation_duration = (te_fwd(1) - te_bwd(1))*1.5;

% Create a handle to the objective function, passing the params struct.
fitness_function = @(x) sma_error_objective(x, ga_params);

% --- 4. Set GA Options ---
nvars = 6; % 6 Keplerian elements
...
fprintf('Running Genetic Algorithm... This may take some time.\n\n');
[optimized_nodein_nb, fval] = ga(fitness_function, nvars, [], [], [], [], lb, ub, [], options);
...
fprintf('==================================================\n');
fprintf('Final objective function value (percentage error): %.6f\n', fval*100);

% --- Propagate with the optimized initial state for verification ---
state_in_optimized_cart = kep2car(optimized_nodein_nb, mu_central_body);

% Define time vector for final, full-resolution verification propagation
time_vector_verif = linspace(0, ga_params.propagation_duration, time_steps)';

% Create the ephemeris handle for this propagation
...
[~, state_out_verif] = propagateNBodyODE3(state_in_optimized_cart(1:3)', state_in_optimized_cart(4:6)', ...
                                        ...
                                        specialPerturbationIDs, []); % No event function

% Get the final state and convert to Keplerian
...
fprintf('\n--- Comparison ---\n');
fprintf('Optimized nodein_nb:  [%.4e, %.4e, %.4f, %.4f, %.4f, %.4f]\n', optimized_nodein_nb);
fprintf('Original nodein_nb:   [%.4e, %.4e, %.4f, %.4f, %.4f, %.4f]\n\n', nodein_nb);
fprintf('Target SMA (from LC):            %.4f km\n', target_sma);
fprintf('Final NB SMA (from optimized):   %.4f km\n', final_sma_optimized);
fprintf('Final Percentage Difference:     %.4f %%\n\n', final_error_percent);

% --- Calculate and Display the Delta-V for the Final Solution ---
fprintf('--- Delta-V Cost for Optimized Solution ---\n');

% Define initial and final states for the Lambert problem
r_initial_lambert = ga_params.original_state_cart(1:3);
v_initial_lambert = ga_params.original_state_cart(4:6);
r_final_lambert   = state_in_optimized_cart(1:3);
v_final_lambert   = state_in_optimized_cart(4:6);

% Use a more refined search for the final verification
dT_days_lambert_verif = linspace(0.001, 0.1, 100000); %finer timestep
min_dv_final = inf;

optimal_tof_days_final = NaN; % Initialize variable to store the optimal ToF

for t_tof = dT_days_lambert_verif
    try
        [v_trans_i, v_trans_f] = izzoLambert(r_initial_lambert, r_final_lambert, t_tof, 0, mu_central_body);
        dv1 = v_trans_i - v_initial_lambert;
        dv2 = v_final_lambert - v_trans_f;
        dv_total = norm(dv1) + norm(dv2);
        if dv_total < min_dv_final
            min_dv_final = dv_total;
            optimal_tof_days_final = t_tof; % capture the optmial time
        end
    catch
        continue;
    end
...
% Convert the final optimal time from days to minutes
optimal_tof_minutes_final = optimal_tof_days_final * 24 * 60;

fprintf('Minimum Delta-V to reach optimized node: %.4f km/s\n', min_dv_final);
fprintf('Optimal Time of Flight: %.4f days  (%.2f minutes)\n', optimal_tof_days_final, optimal_tof_minutes_final);
fprintf('Constraint was max_dv = %.4f km/s\n', ga_params.max_dv);


if final_error_percent <= 1.0
    fprintf('\nSUCCESS: The optimized semi-major axis is within 1%% of the target.\n');
else
    fprintf('\nFAILURE: The optimized semi-major axis is NOT within 1%% of the target.\n');
end
fprintf('==================================================\n');
...
function error = sma_error_objective(kep_in, P)
    % Objective function for the genetic algorithm.
    % 1. Penalizes solutions with delta-V > max_dv.
    % 2. Propagates until an OUTBOUND SOI crossing.
    % 3. Minimizes the error in the final semi-major axis.
    tic
    % =====================================================================
    %  PART 1: DELTA-V CONSTRAINT CHECK (PENALTY METHOD)
    % =====================================================================
    try
        state_in_cart = kep2car(kep_in, P.mu_central_body);
    catch
        error = 1e7; % Penalize invalid Keplerian elements
        return;
    end

    r_initial_lambert = P.original_state_cart(1:3);
    v_initial_lambert = P.original_state_cart(4:6);
    r_final_lambert   = state_in_cart(1:3);
    v_final_lambert   = state_in_cart(4:6);

    % --- Perform a quick Lambert search to find the minimum dV ---
    % Use a coarse search for speed during optimization
    dT_days_lambert = linspace(0.001, 0.1, 1000); 
    min_dv_candidate = inf;
    for t_tof = dT_days_lambert
        try
            [v_trans_i, v_trans_f] = izzoLambert(r_initial_lambert, r_final_lambert, t_tof, 0, P.mu_central_body);
            dv1 = v_trans_i - v_initial_lambert;
            dv2 = v_final_lambert - v_trans_f;
            dv_total = norm(dv1) + norm(dv2);
            if dv_total < min_dv_candidate
                min_dv_candidate = dv_total;
            end
        catch
            % Lambert solver can fail for certain geometries/times. Ignore these.
            continue;
        end
    end

    % --- Apply Penalty if dV is too high ---
    if min_dv_candidate > P.max_dv
        % Assign a large penalty. The penalty increases the further it is
        % from the valid dV, which can help guide the search.
        error = 1e6 + (min_dv_candidate - P.max_dv)^2;
        return;
    end
    ...
    % =====================================================================
    r_start = state_in_cart(1:3)';
    v_start = state_in_cart(4:6)';

    % --- Define propagation settings ---
    time_vector = linspace(0, P.propagation_duration, 2000)'; % Propagate for a long duration

    % --- Create handles for ephemeris and the new outbound event function ---
...
function [value, isterminal, direction] = soiOutboundCrossingEvent(t, x, ephem_enceladus_handle, target_distance)
    % This event function stops integration ONLY when the spacecraft is
    % moving AWAY from Enceladus as it crosses the SOI boundary.

    r_sc = x(1:3);
    r_enceladus = ephem_enceladus_handle(t);

    % The value that will be checked for a zero-crossing
    ...
    % --- KEY CHANGE ---
    % Only trigger if the 'value' function is INCREASING.
    % This means the distance is increasing, so it's an outbound crossing.
    direction = 1;
end
