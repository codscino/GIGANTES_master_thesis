function [yy, tt, RR, VV, lats, longs, vvinfouBM, delta, lat, long] = ...
    groundTrackFlyByMoonClaudio(vvinfin, vvinfou, deltaMax, rr, vv, muPL, pars)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Filter out points where the altitude is greater than hMapping %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Calculate the altitude of every point in the trajectory
altitudes = RR - pars.Moon.EquRad;

% Create a logical index for points within the mapping altitude
mapping_indices = altitudes <= pars.INPUTS.Flyby.hMapping;

% Filter all relevant arrays to keep only the points within mapping altitude
yy_filtered = yy(mapping_indices, :);
RR_filtered = RR(mapping_indices);

% --> compute the groundtracks
lats         = asin( yy_filtered(:,3)./RR_filtered );         % [rad] Latitude of the ground-track points
longs         = atan2(yy_filtered(:,2), yy_filtered(:,1) );   % [rad] Longitude of the ground-track points
