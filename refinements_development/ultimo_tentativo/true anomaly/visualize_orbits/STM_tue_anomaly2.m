function [NB_in_corrected, B_achieved_final, actual_time_diff, deltaV_magnitude, ...
          ...
          STM_tue_anomaly2(pars, nodein, nodeout, suppress_output)
% performSTMOptimization: Use STM to correct initial conditions for B-plane targeting
% Modified to use true anomaly-based backward propagation and return comprehensive results
%
% INPUTS:
...
%   NB_in_state - Original uncorrected initial state vector
%   initial_epoch_nbody - Initial epoch for N-body propagations (MJD2000)
%   initial_epoch_lc - Initial epoch for linked conics (flyby epoch, MJD2000)
%   backward_duration_sec - Backward propagation duration (seconds)
%   mu_TBs - Gravitational parameters for perturbing bodies
%   actualBodyNaifIDs - NAIF IDs of actual perturbing bodies
%   specialPerturbationIDs - Special perturbation IDs (negative values)
%   flyby_states - Structure with flyby states: .r0_sc_in, .v0_sc_in, .r0_sc_out, .v0_sc_out
%   T_linked_conics - Linked conics orbital period (seconds)
%   propagation_duration - Recommended propagation duration (seconds)
%   ephem_handles - Structure with ephemeris function handles
...
if ~suppress_output
    ...
    fprintf('\n=== SUMMARY OF OUTPUTS ===\n');
    fprintf('  Original N-body initial state calculated\n');
    fprintf('  Corrected N-body initial state calculated\n');
    fprintf('  Ephemeris handles created\n');
    fprintf('  All timing and orbital parameters computed\n');
end
...
% Additional outputs for sweep analysis
