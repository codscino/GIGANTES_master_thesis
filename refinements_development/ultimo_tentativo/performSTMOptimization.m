function [NB_in_corrected, B_achieved_final] = performSTMOptimization(pars, nodein, nodeout)
% performSTMOptimization: Use STM to correct initial conditions for B-plane targeting
%
% This function implements an iterative Newton-Raphson scheme using the STM
% to adjust the initial conditions at -64 SOI to achieve the desired B-plane
% parameters at periapsis.
...
pericenter_altitude = rp - pars.Moon.EquRad; % Calculate pericenter altitude
...
% NB_in = ye_bwd(1, :)'; % Initial state at -64 SOI
...
while iteration < max_iterations && ~converged
    ...
    % --- Propagate with STM ---
    flyby_tot_duration = duration_sec * 1.1; % Approximate duration
    ...
    % We need the partial derivatives ∂B/∂X at periapsis
    % This requires mapping through the STM
    
    % Compute ∂B/∂(r_rel, v_rel) at periapsis numerically
    dB_dState = computeBplaneSensitivity(r_sc_rel, v_sc_rel, pars.Moon.mu);
    
    % Map to initial conditions: ∂B/∂X₀ = ∂B/∂X_peri * ∂X_peri/∂X₀
    % The STM gives us ∂X_peri/∂X₀
    dB_dX0 = dB_dState * STM_at_peri;
    
    % Extract relevant sensitivities (we only care about B_R and B_T)
    % and we typically only adjust velocity (indices 4:6)
    H = [dB_dX0(1, 4:6);   % ∂B_R/∂v₀
         dB_dX0(2, 4:6)];  % ∂B_T/∂v₀
    
    % Newton-Raphson correction
    % We want: H * Δv = -[ΔB_R; ΔB_T]
    target_error = -[delta_B_R; delta_B_T];
    
    % Solve for velocity correction (using pseudo-inverse for robustness)
...
deltaV_magnitude = norm(deltaV_vector) * 1000; % Convert to m/s
...
% fprintf('  Applied at: -%.1f SOI from Enceladus\n', soi_multiplier);
