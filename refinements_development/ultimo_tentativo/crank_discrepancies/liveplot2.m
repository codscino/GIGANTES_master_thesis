function liveplot2(state_LC_resampled, r_enc_history, r_titan_history, full_time_out_nb, pars, varargin)
...
% Shows the Linked Conics trajectory with Enceladus and Titan, and optionally the N-body trajectory.
% When N-body is plotted, also displays the live true anomaly of the spacecraft.
%
% INPUTS:
...
%                           'NBBody', state_nb - N-body trajectory state history [N x 6]
%
% EXAMPLE USAGE:
%   % With only Linked Conics:
%   liveplot2(state_LC_resampled, r_enc_history, r_titan_history, full_time_out_nb, pars);
%   
%   % With both Linked Conics and N-body:
%   liveplot2(state_LC_resampled, r_enc_history, r_titan_history, full_time_out_nb, pars, 'NBBody', full_state_out_nb);

% --- Parse Optional Inputs ---
p = inputParser;
addParameter(p, 'NBBody', [], @(x) isempty(x) || (isnumeric(x) && size(x,2) == 6));
parse(p, varargin{:});

state_nb = p.Results.NBBody;
plotNBody = ~isempty(state_nb);
...
% --- Find the initial index for t = 0 minutes ---
[~, initialIndex] = min(abs(time_in_minutes - (-0))); %-200
currentIndex = initialIndex;

% --- Create Figure and Axes ---
fig_title = 'Animated Saturn-Centric Flyby - Linked Conics with Titan and Enceladus';
if plotNBody
    fig_title = [fig_title ' + N-Body'];
end

fig_anim = figure('Name', fig_title, 'Color', 'w', 'Position', [150 150 1000 850]);
...
% Plot orbital paths and spacecraft trajectories
...
plot3(ax_anim, state_LC_resampled(:,1), state_LC_resampled(:,2), state_LC_resampled(:,3), ...
      'Color', [0, 0.4470, 0.7410], 'LineStyle', '--', 'LineWidth', 1, 'DisplayName', 'Linked Conic S/C Trajectory');

% Plot N-body trajectory if provided
if plotNBody
    plot3(ax_anim, state_nb(:,1), state_nb(:,2), state_nb(:,3), ...
          'Color', [0.8500, 0.3250, 0.0980, 0.3], 'LineWidth', 3, 'DisplayName', 'N-Body S/C Trajectory');
end
...
% Spacecraft markers 
h_sc_lc = plot3(ax_anim, NaN, NaN, NaN, 's', 'MarkerSize', 5, 'MarkerFaceColor', [0, 0.4470, 0.7410], 'MarkerEdgeColor', 'k', 'DisplayName', 'Linked Conic S/C');

% N-body spacecraft marker
if plotNBody
...
% --- True Anomaly Display (only if N-body is plotted) ---
if plotNBody
    % Label for true anomaly
    uicontrol('Parent', fig_anim, 'Style', 'text', 'String', 'N-Body True Anomaly:', ...
              'Position', [20, 60, 140, 20], 'HorizontalAlignment', 'left', 'FontSize', 10, 'FontWeight', 'bold');
    
    % Text box for displaying true anomaly value
    h_text_trueanomaly = uicontrol('Parent', fig_anim, 'Style', 'text', 'String', '0.000°', ...
                                   'Position', [20, 35, 140, 25], 'FontSize', 12, 'FontWeight', 'bold', ...
                                   'BackgroundColor', [0.9, 0.9, 0.9], 'HorizontalAlignment', 'center');
    
    % Additional orbital element display
    uicontrol('Parent', fig_anim, 'Style', 'text', 'String', 'Eccentricity:', ...
              'Position', [20, 10, 80, 15], 'HorizontalAlignment', 'left', 'FontSize', 9);
    h_text_ecc = uicontrol('Parent', fig_anim, 'Style', 'text', 'String', '0.000', ...
                           'Position', [100, 10, 60, 15], 'FontSize', 9, ...
...
% --- Helper Functions (Nested) ---
    function updateFrame(k)
        ...
        % Update spacecraft position (Linked Conics)
        set(h_sc_lc, 'XData', state_LC_resampled(k,1), 'YData', state_LC_resampled(k,2), 'ZData', state_LC_resampled(k,3));
        
        % Update spacecraft position (N-body, if plotting)
        if plotNBody && ~isempty(h_sc_nb)
            set(h_sc_nb, 'XData', state_nb(k,1), 'YData', state_nb(k,2), 'ZData', state_nb(k,3));
            
            % Calculate and display true anomaly for N-body trajectory
            try
                % Convert N-body state to Keplerian elements
                kep_nb = car2kep(state_nb(k,:), pars.Planet.mu);
                
                % Extract true anomaly (6th element) and convert to degrees
                true_anomaly_rad = kep_nb(6);
                true_anomaly_deg = rad2deg(true_anomaly_rad);
                
                % Extract eccentricity (2nd element)
                eccentricity = kep_nb(2);
                
                % Update true anomaly display
                set(h_text_trueanomaly, 'String', sprintf('%.3f°', true_anomaly_deg));
                set(h_text_ecc, 'String', sprintf('%.6f', eccentricity));
                
            catch ME
                % In case of any error in orbital element calculation
                set(h_text_trueanomaly, 'String', 'N/A');
                set(h_text_ecc, 'String', 'N/A');
                warning(['Error calculating orbital elements: ' ME.message]);
            end
        end
        
        % Update title and time displays
        trajectory_info = 'Linked Conics';
        if plotNBody
            trajectory_info = [trajectory_info ' + N-Body'];
        end
        set(h_title, 'String', sprintf('Saturn-Centric View - %s | Time: %.2f minutes (%.3f hours)', trajectory_info, time_in_minutes(k), time_in_hours(k)));
