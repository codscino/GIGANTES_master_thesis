...
% solutions.
indexMax_Score=find(Score_ScientificInteres==2); % we seek for low DV transfers with only one type of fly-by.
...
InfoSubSet=[indexValue' numEnceladusFB(indexMax_Score)' Delta_longitude(indexMax_Score)' Delta_V(indexMax_Score)' ]


[~,indexSorted]=sort(InfoSubSet(:,3),'descend');

InfoSubSet=InfoSubSet(indexSorted,:);

% ploting the maximum Diversity  tour
IndexCheck=25397  % This is the exemple to reanalize.
...
%--------------------------------------------------------------------------
% Specific part for the partial COT Equinox case over the south Pole
Flyby_deltaPumb_and_Cranck=zeros(length(NODEOUT_Matrix(:,1)),2);
for indexNode=1:length(NODEOUT_Matrix(:,1))
    ...
    alfa_in = NODEIN(2); alfa_out = NODEIN(2); % Starting and end condition should be the same for Partial COT
    ...
    if NODEIN(3)==-pi
        Flyby_deltaPumb_and_Cranck(indexNode,:)=[0 Delta_Crank_max];
    else
        Flyby_deltaPumb_and_Cranck(indexNode,:)=[0 -Delta_Crank_max];
    end
end
