...
% Every new update of the toolbox should ensure this runs as it should.

% The script can be used as a base to design and visualize ground tracks.
...
pars.INPUTS.V_inf = 4.5;     %[km/s]


Resonances_Enceladus=[3 1];


        VelEnceladus   = sqrt(pars.Planet.mu/pars.Moon.OrbRad(1));     
        %[km/s] Moon Orbital velocity
        [rp_res,ra_res,E_res, alfa_res, ~, trA_moonCrossing] = Tisserand_Resonances(Resonances_Enceladus,pars.INPUTS.V_inf, pars.Moon.OrbRad(1),pars.Planet.mu, pars);
        
        [~,~,~, alfa_res2, ~, trA_moonCrossing] = Tisserand_Resonances([4 1],pars.INPUTS.V_inf, pars.Moon.OrbRad(1),pars.Planet.mu, pars);
...
[rrga, vvga] = approxEphem_CC(pars.INPUTS.idMoon, pars.INPUTS.epoch0, pars.INPUTS.idCentral); % epoch0 provides only a dummy state vector for the moon.
%--------------------------------------------------------------------------
% STEP 1: Chose your starting node - NODEIN DEFINITION
vinfin = pars.INPUTS.V_inf;
alfain = alfa_res; kin = deg2rad(180-30);
...
SetofAvailableOptions=plot_vinfinity_sphere(NODEIN,[],pars);
...
Delta_Alpha=alfa_res2-alfa_res;
Delta_Crank=0;
% Delta_Alpha=0.00324688700524395;
% Delta_Crank=0;
% NODEOut Definition
...
plot_vinfinity_sphere(NODEIN,NODEOUT,pars);
