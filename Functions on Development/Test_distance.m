function [MinDistance]=Test_distance(NextResonance,FullNode,MTitan0, MEnceladus0, TotalTime, pars)
% Tests
%--------------------------------------------------------------------------
...
ToF_leg_indays=FullNode(3);
TrA_Titan=mod(MTitan0+nTitan*(TotalTime+ToF_leg_indays)*3600*24,2*pi);
MEnceladus=FullNode(8);

% Lambert Arc

TrA_Enceladus_arrival=MEnceladus;
...
if DepFlag==5
    ...
    SVatTitan=SVTitan(TrA_Titan);
    r_moon_departure=SVatTitan(1:3);
    v_moon_departure=SVatTitan(4:6);
elseif DepFlag==1
    % Enceladus position & velocity
    TrA_Enceladus_departure=mod(MEnceladus,2*pi);
    SVatEnceladus=SVEnceladus(TrA_Enceladus_departure);
...
% Enceladus position & velocity
SVatEnceladus=SVEnceladus(TrA_Enceladus_arrival);
...
%--------------------------------------------------------------------------
v_sc_departure=FullNode(4:6)+v_moon_departure;
v_sc_arrival=FullNode(9:11)+v_moon_arrival;
...
% Enceladus position & velocity
SV_Enceladus=SVEnceladus(TrA_Enceladus_arrival);
...
MinDistance=min(distance_titan);
