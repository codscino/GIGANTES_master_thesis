...
%--------------------------------------------------------------------------
% This Demo script continues the exploration up to a larger number of
% Enceladus fly-bys. However, since the tree graph grows very fast,
% specially due to the 7:1 resonances which allow for many opportunities to
% encounter with Titan, a Beam Search algorithm is added to keep the number
% of branches expanded under control.
...
AcceptableDV_threshold=0.05; MaxDV_Allowed=0.1; % km/s
TargetEnceladusFB=10;
nConsecutiveTitanFBs=10;
numMaxOfLoops=TargetEnceladusFB+nConsecutiveTitanFBs;
FileNameStore='Saturn_Enceladus_Titan_4GIGANTES_beamSearchExploration.mat';
...
for iE=2:numMaxOfLoops
    ...
    for iN=1:length(treeExploration)
        ...
        indexLeveragingFlyBys=find(Entire_route(:,8)==5);
        numTitanFB(iN)=length(indexLeveragingFlyBys);
        ...
        Diversity_score(iN)=height(unique(Unique_nodes));
        ...
        ToF(iN)=sum(Entire_route(:,7));
    end
    %--------------------------------------------------------------------------
    % BeamFitness=ToF./numEnceladusFB./Diversity_score;
    BeamFitness=Delta_longitude/numTitanFB(iN); % Exploring for large petal rotation
    % BeamFitness=Diversity_score/numTitanFB(iN); % Exploring for large Diversity of Fly-bys.
    undexMaxDV=find(Delta_V<MaxDV_Allowed);
    treeExploration = treeExploration(undexMaxDV);
    numEnceladusFB=numEnceladusFB(undexMaxDV);
    BeamFitness=BeamFitness(undexMaxDV);

    if length(BeamFitness)>BeamWith
        [~,indexSortedBeam]=sort(BeamFitness);
        treeExploration = treeExploration(indexSortedBeam(1:BeamWith));
    end
    %--------------------------------------------------------------------------
...
for iN=1:length(treeExploration)
    ...
    Score_TidalSpread(iN)=height(unique(Unique_nodes))*sum(Travel_matrix(:));
...
%--------------------------------------------------------------------------
% First part for Delta_longitude
%--------------------------------------------------------------------------
% Plot Fitness versus DV and ToF
% Delta_longitude vs Delta V
figure1 = figure;
% Create axes
...
set(axes1,'FontName','Times New Roman','FontSize',12,'FontWeight','bold',...
    'XGrid','on','YGrid','on');
%--------------------------------------------------------------------------
% Delta_longitude vs ToF
figure1 = figure;
% Create axes
axes1 = axes('Parent',figure1);
hold(axes1,'on');
% Create plot
plot(Delta_longitude,Total_time_of_flight,'MarkerFaceColor',[0 0 0],'Marker','o','LineStyle','none');
% Create ylabel
...
%% Plot Fitness versus DV and ToF
%--------------------------------------------------------------------------
% First part for Diversity_score
%--------------------------------------------------------------------------
% Plot Fitness versus DV and ToF
% Delta_longitude vs Delta V
figure1 = figure;
...
hold(axes1,'on');
% Create plot
plot3(Score_ScientificInteres,Total_time_of_flight,Delta_V ,'MarkerFaceColor',[0 0 0],'Marker','o','LineStyle','none');
zlabel('\Deltav [km/s]');
ylabel('Total Time of Flight [days]');
xlabel('\DeltaL [degrees]');
box(axes1,'on');
...
% Plot Best Option
% ploting the maximum longitude tour
[~,indexMax]=max(Score_ScientificInteres);
Full_route=treeExploration{indexMax}.route;
totalDV=sum(treeExploration{indexMax}.routeDVCosts);
routeAnatomy=treeExploration{indexMax}.routeAnatomy;
plotfullSaturnTransfer(Full_route,pars)
